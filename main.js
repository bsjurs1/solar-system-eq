...
const backgroundDarkeningMask = await bodySegmentation.toCanvasImageSource(people, foregroundColor, backgroundColor, drawContour, foregroundThreshold);

const opacity = 0.9;
const maskBlurAmount = 2; // Number of pixels to blur by.
const canvas = document.getElementsByClassName('webgl')[0];

const people2 = await bodySegmentation.drawMask(canvas, image, backgroundDarkeningMask, opacity, maskBlurAmount);
...
//   const pointLight = new THREE.PointLight(0xffffff, 0.7, 500);
...
//   const camControls = new FirstPersonControls(camera, document);
...
//   sunMaterial.emissiveIntensity = 0.4; // Intensity, adjust as needed
...
//       const recomputedRadius =
//         radius + origoCenteredFrequencyIntensity * signalScalingFactor;
...
//     const scalingFactor = computeSunScaleAtAxis(
//       oldSunRadius,
//       frequencyIntensity
//     );
...
// let normalizedFrequencySubtractors = [
//   0.6, 0.6, 0.5, 0.5, 0.4, 0.4, 0.1225, 0.11,
// ];
// const trailPointDisplacementFactors = [20, 30, 40, 40, 50, 50, 40, 30].map(
//   (a) => a * 0.3
// );
...
//       frequencyData[frequencyIndeces[index]] / 256 -
//       normalizedFrequencySubtractors[index];
//     const displacement =
//       normalizedFrequency * trailPointDisplacementFactors[index];
//     trailsPointSizes[index] = normalizedFrequency * 2;
//     positions[0] = planet.position.x + displacement;
//     positions[1] = planet.position.y + displacement;
//     positions[2] = planet.position.z + displacement;
...
//       renderPlanets(
//         planets,
//         frequencyBinDataArray,
//         normalizedFrequencyIntensity
//       );
...
// startAudioVisualization("./miracle.mp3", audioContext);
